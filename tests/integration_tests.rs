...
fn test_integration_load_csv_with_missing_forecast() {
    ...
    let projects_path = temp_dir.join(format!(
    ...
    let forecasts_path = temp_dir.join(format!(
...
fn test_integration_end_to_end_workflow() {
    ...
    let org_a = sales_org_summaries
        .iter()
        .find(|s| s.sales_org == "ORG-A")
        .unwrap();
    // PROJ-001 + PROJ-002: month1 = 10000 + 20000 = 30000
    assert_eq!(org_a.month1_total, 30000.0);

    let org_b = sales_org_summaries
        .iter()
        .find(|s| s.sales_org == "ORG-B")
        .unwrap();
    ...
    let ae1 = ae_summaries
        .iter()
        .find(|s| s.account_executive == "AE 1")
        .unwrap();
    // PROJ-001 + PROJ-002: month1 = 10000 + 20000 = 30000
    assert_eq!(ae1.month1_total, 30000.0);

    let ae2 = ae_summaries
        .iter()
        .find(|s| s.account_executive == "AE 2")
        .unwrap();
    // PROJ-003: month1 = 30000
    ...
    let manager_a = projects_by_pm
        .iter()
        .find(|g| g.project_manager == "Manager A")
        .unwrap();
    assert_eq!(manager_a.projects.len(), 2);

    let manager_b = projects_by_pm
        .iter()
        .find(|g| g.project_manager == "Manager B")
        .unwrap();
